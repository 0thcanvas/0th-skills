...
- [ ] Project-local relative evidence still resolves for existing project claims.
- [ ] Global claims with project-owned relative evidence do not become missing merely because the current agent is in another repo.
- [ ] New global/project claims can record the owner context needed to resolve relative evidence later.
- [ ] Maintenance output shows the resolved owner context for missing or degraded evidence.
...
- [ ] A raw-note pointer moved into an archived/raw location is reported as relocatable using the owner context from Slice 2, not the current cwd.
- [ ] Apply mode performs only source-backed lifecycle or pointer updates and records the repair action.
- [ ] Missing evidence with no known relocation remains a visible `needs_review` condition.
...
- [ ] Existing incident logs produce Memory v2 `incident` claims or brief entries when buckets cross the retro threshold.
- [ ] Imported incident summaries cite incident files but do not copy correction-evidence bodies into runtime claims.
- [ ] Import reads only frontmatter and aggregate bucket metadata; tests cover that correction-evidence bodies are not copied into claims or briefs.
- [ ] Re-running the import is idempotent and does not inflate pattern counts.
...
- [ ] Readiness distinguishes project claims, project tasks, global claims, evidence files, source packs, and repo state.
- [ ] Missing source packs do not imply ordinary claim recall is unavailable.
...
- [ ] Instruction drift between workspace/project instructions and the shared Memory v2 block is reported with the conflicting files.
- [ ] Generated local logs such as `error.log` are either routed to runtime state or reported with an ignore/move recommendation.
- [ ] The check is advisory and does not block normal memory recall.

### 7. Boundary Runtime Eval
Executable fixtures cover the boundary-hardening behavior end to end.
- [ ] Runtime eval covers non-repo advisory, cross-repo global evidence resolution, raw-to-archived relocation, incident import, partial readiness, and instruction/local-artifact hygiene.
- [ ] The prior 11 Memory v2 runtime fixtures still pass.
- [ ] A dogfood report can show whether startup recall needed legacy KB fallback after the boundary fixes.
