...
For each matched stack, plan to exercise the row's Minimum behavior using the tool chain in priority order: Playwright → Browser Kit (`bb-browser` MCP) → computer-use (last fallback, only on agents with computer-use granted).
