...
- For recurring verification, use a stable mounted 1Password Environment directly through the consuming application's loader. Repeated `op run` calls are not a configured environment; reserve `op run --env-file <reference-env-file> -- <consuming command>` for one-off proof or bootstrap fallback and launch one consuming process with it. Use an existing verified ignored `.env` or `.env.local` only for permitted project-scoped development secrets. Contact 1Password only when the local environment is missing, stale, or explicitly being rotated.
- Check only project-scoped paths and metadata; do not inspect secret-file contents or borrow another project's environment. A temporary `op://` reference file contains references only, is gitignored, has mode `600`, and is deleted during teardown.
- Run the actual credential-dependent probe inside the safe runner. A presence-only check does not satisfy proof. If the same configuration will be read repeatedly, require the mounted FIFO instead of repeating terminal-session-scoped `op run` authorization. Before blocking, record each attempted safe runner and its exact sanitized error.
