...
- Try the consuming command through the current app loader, a mounted 1Password Environment `.env`, then an existing project-scoped `op://` reference env with `op run --env-file <reference-env-file> -- <consuming command>`. Use an existing verified ignored `.env` or `.env.local` only for permitted project-scoped development secrets. Contact 1Password only when the local environment is missing, stale, or explicitly being rotated.
