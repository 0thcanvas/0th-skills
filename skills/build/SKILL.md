...
- For managed verification commands whose failures should produce a dossier, wrap the command with `node ${OTH_SKILLS_ROOT:-$HOME/0thcanvas/skills}/scripts/failure-dossier-runner.mjs --run-id <unique-run-id> -- <test command>`; use a fresh `--run-id` per run.
