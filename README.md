...
- Skills that dispatch Codex subagents point to `references/codex-dispatch-fallback.md`,
  which centralizes generic-agent fallback `agent_type`, `model`, and `reasoning_effort`
  pins so unavailable named `0th_*` agents do not silently collapse into main-thread work
