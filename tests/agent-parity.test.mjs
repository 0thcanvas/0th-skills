...
const expectedMirrors = {
  ...
  verifier: {
    ...
    requiredFragments: [
      ...
      "minimum_proof_tier",
      "BLOCKED_REAL_ENV",
      "Browser Kit, the managed wrapper around `bb-browser`",
      "Before relying on `browser_*` tools, run `browser-kit mcp status`",
      "call `browser_tab_list` before opening or navigating",
      "pass a tab to `browser_open`",
      "use `browser_tab_new` only when intentionally creating a fresh tab",
      "references/browser-control-policy.md",
      "real Google Chrome",
      "--provider chrome --profile agent",
      "never silently substitute Brave, Chrome for Testing, managed Chromium, or the in-app browser",
      "computer-use against the Google Chrome app",
      "`browser-kit session open",
      ...
      "Playwright is limited to explicitly hermetic automation",
